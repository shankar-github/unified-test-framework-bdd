...
public class APIBase {
	...
	private void initializeRequestSpec(String baseUri) {
		try {
			requestSpec = new RequestSpecBuilder().setBaseUri(baseUri).addFilter(new RequestLoggingFilter())
	...
	public Response sendRequest(String method, String endpoint, Headers headers, Map<String, Object> params) {
		...
		try {
			...
			case "DELETE":
				...
				break;
			case "HEAD":
				response = given().spec(requestSpec).head(endpoint);
				break;
			case "OPTIONS":
				response = given().spec(requestSpec).options(endpoint);
				break;
			default:
	...
	/**
	 * Logs the response details for debugging and Allure.
	 */
	...
	// Other methods for validation, logging, and data extraction are retained
	// without omissions.
